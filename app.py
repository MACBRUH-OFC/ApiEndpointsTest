...
# Dynamic relative API payload mapping (hex)
...
# Set up reliable HTTP session parameters
...
def get_version_endpoint():
    try:
        ...
        return jsonify({"version": v, "success": True})
...
def run_script():
    try:
        ...
        # Token Retrieval
        token = server_tokens.get(server)
        if not token:
            token = get_token(region_data, release_version)
            if not token:
                return jsonify({"error": "Token generation failed"})
        ...
        try:
            ...
            return jsonify({"error": f"Timeout connecting to {server.upper()} client."})

        # Token Refresh on Auth failure
        if response.status_code == 401:
            token = get_token(region_data, release_version)
            if not token:
                return jsonify({"error": "Refreshed token acquisition failed"})
            server_tokens[server] = token
            headers["Authorization"] = f"Bearer {token}"
            response = SESSION.post(
                url,
        ...
        response.raise_for_status()
        decompressed = decompress_data(response.content)

        if not decompressed:
            return jsonify({"error": "Null decompressed content payload"})

        # Decode via Protobuf Decoder
        protobuf = decode_protobuf(decompressed.hex())

        # Recursively traverse JSON Protobuf response to extract all strings
        extracted_strings = set()

        def extract_strings_from_dict(data):
            if isinstance(data, dict):
                for k, v in data.items():
                    extract_strings_from_dict(v)
            elif isinstance(data, list):
                for item in data:
                    extract_strings_from_dict(item)
            elif isinstance(data, str):
                extracted_strings.add(data)
            elif isinstance(data, (int, float)):
                extracted_strings.add(str(data))

        extract_strings_from_dict(protobuf)

        # Classify asset matches and folder patterns
        urls = set()
        for val in extracted_strings:
            ...
            social_domains = ["instagram.com", "discord.gg", "youtube.com", "facebook.com", "x.com", "linktr.ee"]
        ...
        # Output dynamic, clean formatted Protobuf JSON representation as RAW RESPONSE
        pretty_raw_json = json.dumps(protobuf, indent=2, ensure_ascii=False)
